...
@Component({
  selector: 'app-hole15',
  templateUrl: './hole15.component.html',
  styleUrls: ['./hole15.component.css']
})
export class Hole15Component implements OnInit {
  courseName: any;
  info: any;
  ...
  getCourseDetails() {
    ...
    return firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15').once('value').then(function(snapshot) {
  ...
  saveGeneral() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15');
    courseRef.update({
  ...
  saveBlue() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15/Blue_Square');
  ...
  saveRed() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15/Red_Circle');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  savePink() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15/Pink_Diamond');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  saveYellow() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole15/Yellow_Triangle');
    // push new data to database
    courseRef.update({
      // Save hole description
