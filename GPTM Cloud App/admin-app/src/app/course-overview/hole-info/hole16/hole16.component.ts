...
@Component({
  selector: 'app-hole16',
  templateUrl: './hole16.component.html',
  styleUrls: ['./hole16.component.css']
})
export class Hole16Component implements OnInit {
  courseName: any;
  info: any;
  ...
  getCourseDetails() {
    ...
    return firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16').once('value').then(function(snapshot) {
  ...
  saveGeneral() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16');
    courseRef.update({
  ...
  saveBlue() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16/Blue_Square');
  ...
  saveRed() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16/Red_Circle');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  savePink() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16/Pink_Diamond');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  saveYellow() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole16/Yellow_Triangle');
    // push new data to database
    courseRef.update({
      // Save hole description
