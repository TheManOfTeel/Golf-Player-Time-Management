...
@Component({
  selector: 'app-hole09',
  templateUrl: './hole09.component.html',
  styleUrls: ['./hole09.component.css']
})
export class Hole09Component implements OnInit {
  courseName: any;
  info: any;
  ...
  getCourseDetails() {
    ...
    return firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole9').once('value').then(function(snapshot) {
  ...
  saveBlue() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole9/Blue_Square');
  ...
  saveRed() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole9/Red_Circle');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  savePink() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole9/Pink_Diamond');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  saveYellow() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole9/Yellow_Triangle');
    // push new data to database
    courseRef.update({
      // Save hole description
