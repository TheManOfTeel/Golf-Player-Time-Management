...
export class Hole01Component implements OnInit {
  ...
  getCourseDetails() {
    ...
    return firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole1').once('value').then(function(snapshot) {
  ...
  saveBlue() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole1/Blue_Square');
  ...
  saveRed() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole1/Red_Circle');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  savePink() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole1/Pink_Diamond');
    // push new data to database
    courseRef.update({
      // Save hole description
  ...
  saveYellow() {
    const courseRef = firebase.database().ref('/GolfCourse/' + this.courseName + '/Holes/Hole1/Yellow_Triangle');
    // push new data to database
    courseRef.update({
      // Save hole description
